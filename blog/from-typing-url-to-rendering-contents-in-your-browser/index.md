...
- MACアドレスが自分宛かどうかチェックし、異なる場合捨てる。自分ならバッファメモリに貯める
  - MAC回路の仕事は終了で、コンピュータに割り込みで通知する
- LANアダプタが拡張バススロットにある割り込み用の信号線に信号を送る
- CPUは今の仕事を止めて、割り込み処理用プログラムに切り替える
- LANドライバを呼び出しLANアダプタをコントロールして受信動作を行う
- MACヘッダーのタイプ・フィールドを確認して適切なプロトコルスタックにパケットを渡す
    - 0800だったらIPプロトコル

ここからは、LANアダプタの仕事ではなく、IP担当が処理する範囲です。

- (タイプフィールドが0800でIPが受け取ったと仮定) IPヘッダーを調べて、フォーマットに誤りがないか確認⇨宛先IPアドレスが自分のものか確認
