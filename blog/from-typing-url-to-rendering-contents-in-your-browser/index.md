...
資格自体が欲しいというよりは、基礎体力をつけるために勉強したく、期限と強制力を持たせるために試験を受ける予定なのですが、勉強を進め知っていることが増えていくに従い、詳細に気を取られ全体が見えなくなってきてしまっていました。

そこで、「木を見て森を見ず」な状態を脱却するために一度全体の大きな流れを掴んでから、再度詳細を勉強した方が効率が良いのではないかと思いました。
そこで擦られ倒した話題ではあるものの、**「URLがアドレスバーに入力されてからWebページが表示されるまで」**の流れをまとめていこうと思った次第です。
...
1. サーバーが信号を受信する
   1. ブラウザがデータ送信の際にとったアクションの逆
   2. 信号から元のリクエストメッセージまで戻す
2. サーバーアプリケーションがデータを受け取る
   1.  URI、HTTPメソッドなどの情報をもとに処理
3. サーバーがレスポンスメッセージを作成してブラウザに送り返す
4. ブラウザが信号を受信する
   1. サーバーの受信動作と同じ
5. 信号から元のレスポンスメッセージまで戻す
6. レスポンスヘッダーを確認して、必要であればコンテンツを処理
   1. （HTMLの場合）HTMLファイルを解析してOSにブラウザに表示するよう指示を出す
   2. （画像が含まれる場合）画像ファイルを再度サーバーにリクエストする。
